...
#sourceCpp("empirical_method.cpp")

name <- "Drosophila_filtered"
count_file <- paste0("counts_" ,name, ".cf")     # count file
n_alleles  <- 4                        # the four nucleotide bases A, C, G and T
N          <- 20                        # virtual population size

alignment <- counts_to_pomo_states_converter(count_file,n_alleles,N)

# writing the PoMo alignment
writeLines(alignment,paste0("pomo", N, "_naturalnumbers_", name, ".txt"))
